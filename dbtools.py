...
class Dao(object):
    ...
    def updateQuantity (self, x, id):
        c = self._conn.cursor()
        c.execute('UPDATE products SET quantity={} WHERE id={}'.format(x,id))

    def updateEmployee (self, x, id):
        c = self._conn.cursor()
        c.execute('UPDATE employees SET sales_income={} WHERE id={}'.format(x,id))
    def selectForActivity(self,activities_table,products_table):
        c=self._conn.cursor()
